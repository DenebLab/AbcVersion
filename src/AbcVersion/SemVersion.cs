...
public class SemVersion : IComparable, IComparable<SemVersion>, IEquatable<SemVersion>
{
    ...
    public SemVersion(int major, int minor, int patch, string preRelease, string meta, string versionString)
    {
        ...
        else
        {
            ...
            if (IsPreRelease) sb.AppendFormat(CultureInfo.InvariantCulture, "-{0}", PreRelease);

            if (HasMeta) sb.AppendFormat(CultureInfo.InvariantCulture, "+{0}", Meta);
    ...
    /// </summary>
    public static SemVersion Zero { get; } = new(0, 0, 0, null, null, "0.0.0");
    ...
    {
        return operand1 is not null && operand1.CompareTo(operand2) == 1;
    }
    ...
    public static bool operator <=(SemVersion operand1, SemVersion operand2)
    {
        return operand1 is null || operand1.CompareTo(operand2) <= 0;
    }
    ...
    public static bool operator ==(SemVersion operand1, SemVersion operand2)
    {
        return operand1?.Equals(operand2) ?? operand2 is null;
    }
    ...
    {
        if (TryParse(version, out var semVersion)) return semVersion;
